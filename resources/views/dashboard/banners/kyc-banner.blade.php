<!-- KYC Banner - Mobile First 2025/2026 - URGENT VERSION -->
@php
    $user = auth()->user();
    $provider = $user->serviceProvider ?? null;
    // Afficher le badge si KYC non vérifié OU pas de stripe_account_id
    $needsKyc = $provider && ($provider->kyc_status !== 'verified' || empty($provider->stripe_account_id));
@endphp

@if($user->user_role === 'service_provider' && $needsKyc)
    <style>
        /* ========================================
           KYC BANNER 2025/2026 - URGENT
           ======================================== */
        ...
        @keyframes iconPulse {
            ...
            50% { transform: scale(1.08); box-shadow: 0 6px 16px rgba(245, 158, 11, 0.5); }
        }
    ...
    <div id="kyc-banner" class="kyc-banner-2025" role="alert" aria-live="assertive">
        <div class="kyc-content-wrapper">
            <div class="kyc-icon-wrapper">
                ...
                <div class="kyc-text-content">
                    <h3 class="kyc-title-2025">
                        <span class="kyc-warning-emoji" aria-hidden="true">⚠️</span>
                        Account Verification Required
                    </h3>
                    <p class="kyc-description-2025">
                        <strong>You cannot receive payments</strong> until you complete KYC verification with Stripe<span class="kyc-time-estimate">~5 min</span>
                    </p>
                </div>
            </div>
