...
@php
  ...
  // Phone codes mapping by country short_name (ISO 3166-1 alpha-2)
  $phoneCodes = [
  ...
  <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] flex flex-col" onclick="event.stopPropagation()">
    ...
    <div id="countryCodeList" class="flex-1 overflow-y-auto p-2 sm:p-4">
      @foreach($countries as $country)
        @php $phoneCode = $phoneCodes[strtoupper($country->short_name)] ?? ''; @endphp
        @if(!empty($country->short_name) && !empty($phoneCode))
        <button
          type="button"
          ...
          data-code="{{ $phoneCode }}"
          data-flag="{{ strtolower($country->short_name) }}"
          onclick="selectCountryCode('{{ $country->country }}', '{{ $phoneCode }}', '{{ strtolower($country->short_name) }}')"
        >
          <img src="{{ asset('images/flags/' . strtolower($country->short_name) . '.svg') }}" alt="{{ $country->short_name }}" class="w-6 h-4 object-cover rounded-sm shadow-sm flex-shrink-0">
          <span class="flex-1 font-semibold text-gray-700 text-sm">{{ $country->country }}</span>
          <span class="font-bold text-blue-600 text-sm">{{ $phoneCode }}</span>
        </button>
        @endif
      @endforeach
    </div>
