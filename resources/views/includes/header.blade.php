...
<div id="signupPopup" class="fixed inset-0 bg-black/50 z-50 hidden flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-6">
  ...
  <div class="bg-white w-full h-[100dvh] sm:h-auto sm:max-w-4xl sm:max-h-[90vh] rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl animate-slideUp sm:animate-fadeIn flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto overscroll-contain px-4 sm:px-8 pt-0 pb-20 sm:pb-4" id="popupContentArea">
      ...
      <!-- (unchanged markup except behavior fixed elsewhere) -->
      <!-- BEGIN STEP 5 -->
  ...
  <nav id="mobile-menu" class="lg:hidden fixed top-[64px] left-0 w-full bg-white z-40 shadow-md hidden px-6 py-4 space-y-4" role="navigation" aria-label="Mobile menu" aria-hidden="true">
    ...
    <!-- Removed duplicate #google_translate_element instances -->
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateHeaderButtons() {
    ...
    if (mobileBackBtn) { const span = mobileBackBtn.querySelector('span'); if (span) span.textContent = 'Back'; }
    if (desktopBackBtn) { const span = desktopBackBtn.querySelector('span'); if (span) span.textContent = 'Back'; }

    if (currentStep === 3) {
      const hasServices = window.selectedServices && Object.keys(window.selectedServices).length > 0;
      const servicesWithSubcats = window.servicesData?.filter(s => 
        window.selectedServices[s.serviceId] && s.subcategories && s.subcategories.length > 0
      ) || [];
      const allSubcatsSelected = servicesWithSubcats.every(service => 
        window.selectedSubcategories && window.selectedSubcategories[service.serviceId] && window.selectedSubcategories[service.serviceId].length > 0
      );
      let buttonText = 'Continue';
      if (hasServices && servicesWithSubcats.length > 0 && !allSubcatsSelected) {
        buttonText = 'Choose Sub categories';
      }
      if (mobileNextBtn) { const span = mobileNextBtn.querySelector('span'); if (span) span.textContent = buttonText; }
      if (desktopNextBtn) { const span = desktopNextBtn.querySelector('span'); if (span) span.textContent = buttonText; }
      return;
    }

    const defaultText = isLastStep ? 'Finish' : 'Continue';
    if (mobileNextBtn) { const span = mobileNextBtn.querySelector('span'); if (span) span.textContent = defaultText; }
    if (desktopNextBtn) { const span = desktopNextBtn.querySelector('span'); if (span) span.textContent = defaultText; }
  }
  ...
  function validateStep(index) {
    switch(index) {
      case 1: return typeof window.validateStep2 === 'function' ? window.validateStep2() : !!document.querySelector('#step2 .language-card.selected');
      case 2: return typeof window.validateStep3 === 'function' ? window.validateStep3() : document.querySelectorAll('#step3 .lang-btn.selected').length > 0;
      case 3:
      ...
      case 4: return typeof window.validateStep5 === 'function' ? window.validateStep5() : document.getElementById('location-input')?.value.trim().length > 0;
      case 5: return typeof window.validateStep6 === 'function' ? window.validateStep6() : document.querySelectorAll('#countryList input[type="checkbox"]:checked').length >= 1;
      case 6: return typeof window.validateStep7 === 'function' ? window.validateStep7() : Array.from(document.querySelectorAll('#step7 .special-status-item'));
      case 7: return typeof window.validateStep8 === 'function' ? window.validateStep8() : Array.from(document.querySelectorAll('#step8 .speak-toggle')).every(group => group.querySelector('.bg-green-500'));
      case 8: return typeof window.validateStep9 === 'function' ? window.validateStep9() : document.getElementById('profileDescription')?.value.trim().length > 0;
      case 9: return typeof window.validateStep10 === 'function' ? window.validateStep10() : document.getElementById('profileUpload')?.files.length > 0;
      case 11: return typeof window.validateStep12 === 'function' ? window.validateStep12() : document.getElementById('first_name_input')?.value.trim().length > 0;
      case 12: return typeof window.validateStep13 === 'function' ? window.validateStep13() : document.getElementById('email_input')?.value.trim().length > 0;
      case 13:
        if (typeof window.validateStep14 === 'function') return window.validateStep14();
        const phoneInput = document.getElementById('phone_number_input');
        if (phoneInput && phoneInput.iti) return phoneInput.iti.isValidNumber();
        return phoneInput && phoneInput.value.trim().length > 0;
      case 14: return typeof window.validateStep15 === 'function' ? window.validateStep15() : document.getElementById('otp_input')?.value.trim().length > 0;
      default: return true;
    }
  }
  ...
  function handleNext() {
    ...
    if (currentStep === 3 && typeof window.validateStep4 === 'function') {
      ...
      if (result === 'show_subcategories') {
        if (typeof window.openSubcategoriesModal === 'function') window.openSubcategoriesModal();
        return;
      } else if (!result) { return; }
    }
    if (currentStep < steps.length - 1) { showStep(currentStep + 1); } else { /* submit */ }
  }

  function handleBack() { if (currentStep > 0) showStep(currentStep - 1); }
  ...
  // Step 2 + 3 legacy handlers (unchanged)
  ...
  function openMobileMenu() {
    ...
    mobileMenu.classList.add("menu-open"); // <-- critical for content-visibility
  ...
  function closeMobileMenu() {
    ...
    mobileMenu.classList.add("menu-animating");
    mobileMenu.classList.remove("menu-open"); // <-- critical to hide with content-visibility
...
<script>
  function toggleExpatPopup() { const popup = document.getElementById("expat-popup"); popup.classList.toggle("hidden"); }
  function toggleCategoryPopup() { const popup = document.getElementById("search-category-popup"); popup.classList.toggle("hidden"); }
</script>

<script>
  function goBackToMainCategories() { closeAllPopups(); openHelpPopup(); }
  function closeAllPopups() {
    const popups = [
      'searchPopup','expatriesPopup','vacanciersPopup','vacanciersPreparationPopup','vacanciersUrgencePopup',
      'vacanciersProblemesVoyagesPopup','expatriesUrgencePopup','expatriesPreparationPopup','expatriesAssurancePopup',
      'expatriesBesoinsPopup','expatriesSantePopup',"vacanciersAutresBesoinsPopup",'investisseursPopup',
      'investisseurBienImmobilierPopup',"Acheter un bien immobilier","investirMarchesFinanciersPopup",
      "investisseurSecuriserInvestissementsPopup","investisseurOptimisationFiscalePopup","investisseurObligationsLegalesPopup",
      "travailleursFreelancesPopup","travailleursCreerEntreprisePopup","travailleursDevelopperReseauPopup",
      "travailleursGestionFinancierePopup","travailleursProtectionSocialePopup","travailleursTrouverEmploiPopup",
    ...
    popups.forEach(id => { const el = document.getElementById(id); if (el) el.classList.add('hidden'); });
  ...
  function enableButtonIfValid(buttonId, conditionMet) {
    ...
    if (!button) return;
    if (conditionMet) {
      button.disabled = false; button.classList.remove('btn-disabled'); button.classList.add('btn-enabled');
    } else {
      button.disabled = true; button.classList.add('btn-disabled'); button.classList.remove('btn-enabled');
    }
  }

  document.addEventListener('click', function(event) {
    const popups = [ document.getElementById('searchPopup'), document.getElementById('expatriesPopup'), document.getElementById('vacanciersPopup') ];
...
function openHelpPopup() {
    ...
    .then(data => {
      if (data.success) {
        ...
        data.categories.forEach(cat => {
          ...
          div.querySelector('h3').textContent = cat.name;
 ...
 function handleCategoryClick(categoryId, categoryName) {
  ...
  createRequest.category = JSON.stringify({ id: categoryId, name: categoryName });
    ...
    .then(data => {
      if (data.success) {
        ...
        data.subcategories.forEach(sub => {
          ...
          div.innerHTML = `
            ${iconHtml}
            <div class="flex-grow font-semibold  text-gray-800"></div>
          ...
          `;
          div.querySelector('.flex-grow').textContent = sub.name;
    ...
    .then(data => {
      if (data.success) {
        ...
        data.subcategories.forEach(child => {
          ...
          `;
          div.querySelector('.flex-grow').textContent = child.name;
